...
function CreatePostModal() {
    ...
    return (
        <div>
            <input type="checkbox" id="new-post-modal" className="modal-toggle" checked={modalOpen} onChange={(e) => setModalOpen(e.target.checked)} />
            <label htmlFor="new-post-modal" className="cursor-pointer modal">
                <label className="relative modal-box" htmlFor="">
                    ...
                    <form onSubmit={submit}>
                        <textarea className="w-full max-w-md max-h-md textarea-md textarea-bordered textarea" value={postMessage} onChange={(e) => setPostMessage(e.target.value)} placeholder="What do you want to tell the world?"></textarea>
                        <div className="flex flex-row-reverse mt-2 mb-0 mr-1">
                            <button type="submit" disabled={submitDisabled} className="flex px-10 text-white justify-self-end btn btn-primary btn-md hover:text-white">Post</button>
                        </div>
                    </form>
                </label>
            </label>
